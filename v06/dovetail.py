...
This module implements riffle shuffling as described in the 1992 Bayer-Diaconis paper 'Trailing the
Dovetail Shuffle to its Lair', which describes the earlier Gilbert-Shannon-Reeds shuffle and how many
are required for adequate randomization.

To my layman's eyes, it appears they come to the conclusion that after shuffling a deck of N cards
[3/2 * log(base 2)N]-times, shuffling becomes 'effective'. The default number of these 'effective
shuffles' in the function shuffle() is 3, but the kwarg 'eff_shuffles' can be passed to alter this.
